program f_frand123Double
   ...
   ! time frand123Double for scalar results
   call time_frand123Double_scalar_runner( state, compileCombination )

   ! time frand123Double for chunked results
   call time_frand123Double_runner( state, compileCombination )

contains
   subroutine time_frand123Double_scalar( state, num, timeElapsed, resultSum )
      use, intrinsic :: iso_c_binding, only: c_int64_t, c_double
      use :: frand123, only: frand123_state_kind, frand123_state_size, frand123Double
      use :: omp_lib
      implicit none
      ...
      do i = 0, num
         call frand123Double( state, randomNumber )
         dummySum = dummySum + randomNumber
      enddo
      endTime = omp_get_wtime()
      timeElapsed = endTime - startTime
      resultSum = dummySum
   end subroutine time_frand123Double_scalar

   subroutine time_frand123Double_scalar_runner( state, compileCombination )
      use, intrinsic :: iso_c_binding, only: c_int64_t, c_double
      implicit none
      integer( kind = frand123_state_kind ), dimension( frand123_state_size ), intent( inout ) :: state
      ...
      ! create filename
      write( filename, '( "tuning/timings/frand123Double_scalar_f", A, ".dat" )' ) trim( compileCombination )

      ! open file
      open( newunit = fileHandler, file = filename, action = 'write', iostat = stat )
      if( stat .ne. 0 ) then
         write(*,*) 'error opening file in time_frand123Double_scalar_runner Fortran version'
         stop
      endif
      ...
      do i = 1, 6
         ...
         maxTime = 0._c_double
         do k = 0, 20
            call time_frand123Double_scalar( state, localNum, timeElapsed, resultSum )
         ...
         write( fileHandler, '( "frand123Double_scalar,", I12, ",", E13.6, ",", E13.6, ",", E13.6 )' ) &
                & localNum, meanTime, minTime, maxTime
         write( *, '( "frand123Double_scalar,", I12, ",", E13.6, ",", E13.6, ",", E13.6 )' ) & 
                & localNum, meanTime, minTime, maxTime
      enddo
      
      ! close file again
      close( fileHandler )
   end subroutine time_frand123Double_scalar_runner

   subroutine time_frand123Double( state, num, chunksize, timeElapsed, resultSum )
      use, intrinsic :: iso_c_binding, only: c_int64_t, c_double
      use :: frand123, only: frand123_state_kind, frand123_state_size, frand123Double
      use :: omp_lib
      implicit none
      ...
      if( .not. allocated( randomNumber ) ) then
         write(*,*) 'error allocating randomNumber in time_frand123Double Fortran version'
         stop 1
      endif
   ...
   end subroutine time_frand123Double

   subroutine time_frand123Double_runner( state, compileCombination )
      use, intrinsic :: iso_c_binding, only: c_int64_t, c_double
      implicit none
      integer( kind = frand123_state_kind ), dimension( frand123_state_size ), intent( inout ) :: state
      character( len = 100 ), intent( in ) :: compileCombination
      ...
      ! create filename
      write( filename, '( "tuning/timings/frand123Double_f", A, ".dat" )' ) trim( compileCombination )

      ! open file
      open( newunit = fileHandler, file = filename, action = 'write', iostat = stat )
      if( stat .ne. 0 ) then
         write(*,*) 'error opening file in time_frand123Double_runner Fortran version'
         stop
      endif
      ...
      do i = 1, 6
         do j = 1, 8
            ...
            if( localChunksize .le. localNum ) then
               ...
               maxTime = 0._c_double
               do k = 0, 20
                  call time_frand123Double( state, localNum, localChunksize, timeElapsed, resultSum )
                  meanTime = meanTime + timeElapsed
                  minTime = min( minTime, timeElapsed )
                  maxTime = max( maxTime, timeElapsed )
               enddo
               meanTime = meanTime / 20._c_double
               write( fileHandler, '( "frand123Double,", I12, ",", I12, ",", E13.6, ",", E13.6, ",", E13.6 )' ) &
                      & localNum, localChunksize, meanTime, minTime, maxTime
               write( *, '( "frand123Double,", I12, ",", I12, ",", E13.6, ",", E13.6, ",", E13.6 )' ) &
                      & localNum, localChunksize, meanTime, minTime, maxTime
            endif
         enddo
      enddo
      ...
      ! close file again
      close( fileHandler )
   end subroutine time_frand123Double_runner

end program f_frand123Double
