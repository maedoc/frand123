...
#include "frand123enlarger.h"
   ...
   /*
    * Function wichura4x32kernel represents the kernel of the single precision
    * AS 241 algorithm extracted for testing (PPND7)
    */
   void wichura4x32kernel( const float *p, const float mu, const float sigma, float *res )
   {
      ...
      cond[ 2 ] = ( fabs( q[ 2 ] ) <= split1 );
      cond[ 3 ] = ( fabs( q[ 3 ] ) <= split1 );
      // run vectorized version of the main route
      // in the following cases
      // all four values of |q| <= split1: probability ~52%
      // one |q| <= split1, rest |q| > split1: probability ~37%
      // two |q| <= split1, two  |q| > split1: probability ~10%
      // three |q| <= split1, one |q| > split1: probability ~1%
      // all |q| <= split1: probability ~0%
      ...
      // return in ~52% of calls
      if( cond[ 0 ] && cond[ 1 ] && cond[ 2 ] && cond[ 3 ] )
      {
         return;
      }
      // handle first non-standard case
      // probability for need to handle single case: ~37%
      // probability for need to handle two cases: ~10%
      // probability for need to handle three cases: ~1%
      // probability for need to handle all cases: ~0%
      for( i = 0; i < 4; i++ )
      {
         if( ! cond[ i ] )
         {
   ...
   /*
    * Function wichura4x32 calculates four single precision random numbers
    * normally distributed with expectation mu and variance sigma using the
    * inverse transform sampling introduced by Wichura 1988 in algorithm AS241 (PPND7)
    *
    * Arguments: state: four elements holding
    *                   counter: first  128 bit
    *                   key:     second 128 bit
    ...
    *            res:   address to storage for 4 single precision reals
    */
   void wichura4x32( int64_t *state, const float mu, const float sigma, float *res )
   {
      ...
      ars4x32_u01( state, p );
#else
      threefry4x32_u01( state, p );
#endif
      // rest is computed within the kernel
      wichura4x32kernel( p, mu, sigma, res );
      return;
   }
