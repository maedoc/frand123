...
! generation of single precision normally distributed random numbers.
...
program kurtosisSingle
   ...
   real( kind = c_float ), parameter :: mu = real( 0.7, c_float )
   ...
   ! result variables
   real( kind = c_float ) :: varScalar, varTwo, varFour, var1000
   ...
   ! use excessKurtosisFour
   startTime = omp_get_wtime()
   varFour = excessKurtosisFour( state )
   endTime = omp_get_wtime()
   write(*, '( "Result generating four random number at a time: ", ES11.4, &
          " took ", ES11.4, " seconds" )' ) varFour, endTime - startTime
...
contains
   ...
   ! estimate excess kurtosis by drawing a single single precision sample at a
   ! time
   real( kind = c_float ) function excessKurtosisScalar( state )
      ...
      real( kind = c_float ) :: draw
      integer( kind = ik ) :: i

      ! iterate over samples
      excessKurtosis = real( 0, c_float )
      do i = 1, numberOfSamples
         ! draw random number
         call frand123NormSingle( state, mu, sigma, draw )
         ! centralize
         draw = draw - mu
      ...
      excessKurtosisScalar = excessKurtosis / &
          ( real( numberOfSamples, c_float) * sigma ** 4 ) - real( 3., c_float )
   ...
   real( kind = c_float ) function excessKurtosisTwo( state )
      ...
      ! local variables
      real( kind = c_float ) :: excessKurtosis
      real( kind = c_float ), dimension( 2 ) :: draw
      integer( kind = ik ) :: i
   ...
   real( kind = c_float ) function excessKurtosisFour( state )
      ...
      ! local variables
      real( kind = c_float ) :: excessKurtosis
      real( kind = c_float ), dimension( 4 ) :: draw
      integer( kind = ik ) :: i

      ! iterate over samples
      excessKurtosis = real( 0, c_float )
      do i = 1, numberOfSamples, 4
         ! draw random number
         call frand123NormSingle( state, mu, sigma, draw )
         ! centralize
         draw = draw - mu
      ...
      enddo
      ! apply remaining operations
      excessKurtosisFour = excessKurtosis / &
          ( real( numberOfSamples, c_float) * sigma ** 4 ) - real( 3., c_float )
   end function excessKurtosisFour
   ...
   real( kind = c_float ) function excessKurtosis1000( state )
      ...
      ! local variables
      real( kind = c_float ) :: excessKurtosis
      real( kind = c_float ), dimension( 1000 ) :: draw
      integer( kind = ik ) :: i
